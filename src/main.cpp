...
uint256 hashGenesisBlock("0x00000b1903bc3f863147577a730dfe7f1ed9e71f873995d74e758387185a669e"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ConspiracyCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ConspiracyCoin Signed Message:\n";
...
{
    // ConspiracyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // ConspiracyCoin: 1 day
static const int64 nTargetSpacing = 2 * 60; // ConspiracyCoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ConspiracyCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, conspiracycoin - DarkGravity v2, written by Evan Duffield - evan@conspiracycoin.io */
    ...
    if ( ((pindexLast->nHeight-1) % 5000 == 0 || (pindexLast->nHeight) % 5000 == 0 || (pindexLast->nHeight+1) % 5000 == 0) && pindexLast->nHeight > 52000) {
              bnNew = bnProofOfWorkLimit;               
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ConspiracyCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ConspiracyCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Obama in Asia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1398726345;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 29864396;
        ...
        assert(block.hashMerkleRoot == uint256("0xe59cc74d79ab8b8a37f0231b4c52eb326ddaa020f24a7c39d612109f1b86b5d2"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ConspiracyCoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    txNew.vout.resize(1);
     ...
     if ( pindexPrev->nHeight % 5000 == 0 && pindexPrev->nHeight > 1 ){
    ...
    int64 nFees = 0;
    {
        ...
        if ( pindexPrev->nHeight % 5000 == 0 && pindexPrev->nHeight > 1)
        	pblock->vtx[0].vout[0].nValue = FOUNDATION_SUPPORT * COIN;
        else 	
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ConspiracyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ConspiracyCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ConspiracyCoinMiner(CWallet *pwallet)
{
    printf("ConspiracyCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("conspiracycoin-miner");
    ...
    try { loop {
        ...
        printf("Running ConspiracyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ConspiracyCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ConspiracyCoinMiner, pwallet));
}
